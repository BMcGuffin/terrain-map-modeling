...
bool WindowManager::init(int const width, int const height)
{
   ...
   glfwSetCursorPosCallback(windowHandle, motion_callback);
...
void WindowManager::motion_callback(GLFWwindow * window, double dX, double dY)
{
	if (instance && instance->callbacks)
	{
		instance->callbacks->motionCallback(window, dX, dY);
	}
}
